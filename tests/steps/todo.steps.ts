...
When(
  'I add a todo item {string} with type {string}',
  async ({ page }, taskText: string, todoType: string) => {
    const todoPage = new TodoPage(page);
...
Then(
  'the todo item {string} should include type {string}',
  async ({ page }, taskText: string, todoType: string) => {
    const todoPage = new TodoPage(page);
